function Test-FinOpsAzTenantAccess {
    ...
    if ($armToken) {
        _recordStep -Name 'ListSubscriptions' -Action {
            ...
            Invoke-RestMethod -Uri $uri -Headers @{ Authorization = "Bearer $armToken" } -TimeoutSec $TimeoutSeconds -Method Get -ErrorAction Stop | Select-Object -Property value, nextLink
        } | Out-Null
    }
    ...
    if ($kvToken -and $TestVaultName -and $TestSecretName) {
        _recordStep -Name 'GetSecret' -Action {
            ...
            $resp = Invoke-RestMethod -Uri $uri -Headers @{ Authorization = "Bearer $kvToken" } -TimeoutSec $TimeoutSeconds -Method Get -ErrorAction Stop
            $secretRead = $true
