    ...
    process {
        for ($i = 0; $i -lt $EmailAddress.Count; $i++) {
            try {
                ...
                $existingUsers = Invoke-FinOpsRestMethodWithRetry -Uri $checkUri -Headers $headers -Method Get -ErrorAction Stop
                ...
                if ($PSCmdlet.ShouldProcess($email, "Invite guest user to tenant")) {
                    ...
                    $response = Invoke-FinOpsRestMethodWithRetry -Uri $uri -Headers $headers -Method Post -Body $body -ContentType 'application/json' -ErrorAction Stop
