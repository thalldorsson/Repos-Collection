function Invoke-FinOpsJiraPut {
    <#
    .SYNOPSIS
        Internal helper to invoke a Jira Cloud / Server REST PUT request.
    .DESCRIPTION
        Supports either:
    ...
    .PARAMETER RelativePath
        REST path beginning with / (e.g. /rest/api/3/issues/ABC-123)
    .PARAMETER Body
    ...
    .EXAMPLE
        ...
        Invoke-FinOpsJiraPut -BaseUrl https://contoso.atlassian.net -RelativePath "/rest/api/3/issues/ABC-1" -Body $body -Username user@contoso.com -ApiToken $token
    #>
    [CmdletBinding()]
    param(
    ...
    try {
        Invoke-FinOpsRestMethodWithRetry -Uri $uri -Method Put -Headers $headers -Body $bodyJson -MaxRetries $MaxRetries -InitialDelaySeconds $InitialDelaySeconds -ErrorAction Stop
    }
    catch {
        $baseMsg = "Jira PUT failed: $uri"
        $ex = $_.Exception
        $statusLine = ''
        $bodySnippet = ''
        $parsedMessages = ''
