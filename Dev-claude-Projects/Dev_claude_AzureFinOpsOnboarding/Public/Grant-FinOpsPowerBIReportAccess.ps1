    ...
    try {
        ...
        if ($UsePowerBIMcp) {
            ...
            if ($EntraGroupId) {
                ...
                if (-not $ctx) {
                    Connect-MgGraph -Scopes Group.Read.All -NoWelcome -ErrorAction Stop | Out-Null
                }
        ...
        if (-not $token) {
            Write-Verbose "Connecting to Power BI Service Account (interactive)"
        ...
        if ($EntraGroupId) {
            ...
            try { $ctx = Get-MgContext -ErrorAction SilentlyContinue } catch {}
            if (-not $ctx) {
                Write-Verbose "Connecting to Microsoft Graph (Group.Read.All)"
                try {
                    Connect-MgGraph -Scopes Group.Read.All -NoWelcome -ErrorAction Stop | Out-Null
                    $ctx = Get-MgContext
                } catch {
